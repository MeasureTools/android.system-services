...
public class MemoryUsageSupplierService extends ValueSupplierService  {

	private ByteBuffer valueByteBuffer;

	public MemoryUsageSupplierService() {
		super("ODROID_XU3_MEMORY_USAGE");
	...
	private Double readValue() {
		...
		try {
			bufReader = new BufferedReader(new FileReader("/proc/meminfo"));
			String[] total = bufReader.readLine().split("\\s+");
			String[] free  = bufReader.readLine().split("\\s+");
			bufReader.close();
			return Double.valueOf(free[1]) / Double.valueOf(total[1]);
		}
		catch (Exception e) {
